				...
				if(mysqli_num_rows($result) > 0){
					...
					while ($row = mysqli_fetch_array($result)) {
						...
						if($row['availableroom'] != null && $row['availableroom'] > 0  )
						{
							...
							$sub_result2 = mysqli_query($dbhandle,"select room.* from room where room.room_id = ".$row['room_id']." AND location='".$_SESSION['location_entered']."' ");
							if(mysqli_num_rows($sub_result2) > 0)
							{
								...
								print "						<span class=\"fontgrey\">Location : </span> ".$sub_row2['location']."</p>\n";
								print "\n";
								print "						</div>\n";
								print "						<div class=\"large-4 columns\">\n";
								print "						<p ><span class=\"fontgrey\">Rate : $ </span><span style=\"font-size:24px;\">".$sub_row2['rate']."</span><span class=\"fontgrey\">/ night</span><br>\n";
